...
function ux_builder_register_breakpoints() {
  ...
  add_ux_builder_breakpoint( 'sm', 550,  '手机',  'dashicons dashicons-smartphone' );
  add_ux_builder_breakpoint( 'md', 850,  '平板',  'dashicons dashicons-tablet' );
  add_ux_builder_breakpoint( 'lg', 1000, '桌面', 'dashicons dashicons-desktop' );
...
function ux_builder_page_row_actions( $actions, $page_object ) {
	...
	if ( array_key_exists( $page_object->post_type, $post_types ) && current_user_can( 'edit_post', $page_object->ID ) ) {
        array_splice($actions, 1, 0, '<a href="' . ux_builder_edit_url( $page_object->ID ) . '">' . __( '使用四亩地生成器编辑' ) . '</a>' );
	}
