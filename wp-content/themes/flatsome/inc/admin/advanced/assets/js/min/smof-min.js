jQuery.noConflict(), jQuery(document).ready(function ($) {
    ...
    function i(e, t) {
        var i = $(e).val(),
            ...
            o = t + "_ggf_previewer";
        if (i)
            if ($("." + o).fadeIn(), "none" !== i && "Select a font" !== i) {
                $("." + a).remove();
                var n = i.replace(/\s+/g, "+");
				$("head").append(''), $("." + o).css("font-family", i + ", sans-serif")
    ...
    if (jQuery(".fld").click(function () {
            ...
            expires: 7,
            path: "/"
        }), null === $.cookie("of_current_opt")) $(".group:first").fadeIn("fast"), $("#of-nav li:first").addClass("current");
    else {
        var r = $("#hooks").html();
    ...
    $("#of-nav li a").click(function (e) {
        ...
        return $.cookie("of_current_opt", t, {
            expires: 7,
            path: "/"
        }), $(".group").hide(), $(t).fadeIn("fast"), !1
        ...
        var e = $(this).prev(),
            t = e.attr("id"),
            i = $("#" + t + " li").find(".order").map(function () {
                var e = this.id;
        ...
        1 > a && (a = 0);
        var o = a + 1,
			r = '<li class="temphide"><div class="slide_header"><strong>幻灯片' + o + '</strong><input type="hidden" class="slide of-input order" name="' + t + "[" + o + '][order]" id="' + t + "_slide_order-" + o + '" value="' + o + '"><a class="slide_edit_button" href="#">编辑</a></div><div class="slide_body" style="display: none; "><label>标题</label><input class="slide of-input of-slider-title" name="' + t + "[" + o + '][title]" id="' + t + "_" + o + '_slide_title" value=""><label>图片URL</label><input class="upload slide of-input" name="' + t + "[" + o + '][url]" id="' + t + "_" + o + '_slide_url" value=""><div class="upload_button_div"><span class="button media_upload_button" id="' + t + "_" + o + '">上传</span><span class="button remove-image hide" id="reset_' + t + "_" + o + '" title="' + t + "_" + o + '">删除</span></div><div class="screenshot"></div><label>链接URL (可选)</label><input class="slide of-input" name="' + t + "[" + o + '][link]" id="' + t + "_" + o + '_slide_link" value=""><label>描述 (可选)</label><textarea class="slide of-input" name="' + t + "[" + o + '][description]" id="' + t + "_" + o + '_slide_description" cols="8" rows="8"></textarea><a class="slide_delete_button" href="#">删除</a><div class="clear"></div></div></li>';
        e.append(r);
		...
		var e = confirm("'警告：您所有的当前选项将被上次备份的数据替换！ 继续？");
        if (e) {
            var t = $(this),
                i = $(this).attr("id"),
                a = $("#security").val(),
        ...
        if (e) {
            ...
            $.post(ajaxurl, n, function (e) {
                var t = $("#of-popup-fail"),
                    i = $("#of-popup-save"); - 1 == e ? (t.fadeIn(), window.setTimeout(function () {
                    t.fadeOut()
		...
		var e = confirm("点击确定重置。所有设置都将丢失，并被默认设置取代！");
        if (e) {
            var t = $("#security").val();
            ...
            $.post(ajaxurl, i, function (e) {
                var t = $("#of-popup-reset"),
                    i = $("#of-popup-fail"),
                    a = $(".ajax-reset-loading-img");
        ...
        var e = jQuery(this),
            t = "#" + e.data("id"),
            i = parseInt(e.data("val")),
            a = parseInt(e.data("min")),
