...
//四亩地    if ( empty( $purchase_code ) ) {
 //四亩地    echo '<form class="wupdates_purchase_code" action="" method="post">' .
 //四亩地             __( '<p>Enter your purchase code and <strong>hit return/enter</strong>. Find out how to <a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank">get your purchase code</a>.</p>' ) .
 //四亩地             '<input type="hidden" name="wupdates_pc_theme" value="' . $slug . '" />' .
  //四亩地            '<input type="text" id="' . sanitize_title( $slug ) . '_wup_purchase_code" name="' . sanitize_title( $slug ) . '_wup_purchase_code"
  //四亩地             value="' . $purchase_code . '" placeholder="Purchase code ( e.g. 9g2b13fa-10aa-2267-883a-9201a94cf9b5 )" style="width:100%; padding:10px;"/><br/><br/><input type="submit" class="button button-large button-primary" value="Activate"/>
  //四亩地     </form>';
  //四亩地 	} else{
  //四亩地   echo '<form class="wupdates_purchase_code" action="" method="post">' .
  //四亩地            '<input type="hidden" name="wupdates_pc_theme" value="' . $slug . '" />' .
  //四亩地            '<input type="text" id="' . sanitize_title( $slug ) . '_wup_purchase_code" name="' . sanitize_title( $slug ) . '_wup_purchase_code"
  //四亩地             value="' . $purchase_code . '" placeholder="Purchase code ( e.g. 9g2b13fa-10aa-2267-883a-9201a94cf9b5 )" style="width:100%; padding:10px;"/><br/><br/><input type="submit" class="button button-large button-primary" value="Update"/>
  //四亩地     </form>';
  //四亩地 	}
?>
  ...
  <?php
    ...
    if ( flatsome_is_support_expired( $slug ) ) {
	    $support_message = flatsome_is_invalid_support_time( $support_ends ) ? '<strong style="color:orange;">无效（请尝试重新更新）</strong>' : '<strong style="color:red;">过期</strong>';
    }
