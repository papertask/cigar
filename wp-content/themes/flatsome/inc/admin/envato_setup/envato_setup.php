	...
	class Envato_Theme_Setup_Wizard {
		...
		public function admin_redirects() {
			...
			if(flatsome_is_upgrading()){
				wp_safe_redirect( admin_url('admin.php?page=flatsome-panel-support') );//四亩地
			} else {
...
//				print_r($categories);
				...
				<p>在分发/更新主题时，将这些值复制到PHP代码中。</p>
				<?php
			} else if( $this->is_possible_upgrade() ){
				?>
				<h1><?php printf( __( '欢迎使用%s安装向导.' ), wp_get_theme()); ?></h1>
				<p><?php _e('似乎最近您可能升级到了此主题。 太好了！ 此设置向导将帮助您确保所有默认设置都正确。它还将显示有关您新网站和支持选项的一些信息。');?></p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"><?php _e( '开始吧' ); ?></a>
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(),'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
					   class="button button-large"><?php _e( '现在还不行' ); ?></a>
				</p>
				<?php
			} else if( get_option('envato_setup_complete', false) ){
				?>
				<h1><?php printf( __( '欢迎使用%s安装向导.' ), wp_get_theme()); ?></h1>
				<p class="lead success"><?php _e('看起来你已经设置过Flatsome了。');?></p>

				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						   class="button-primary button button-next button-large"><?php _e( '再次运行安装向导' ); ?></a>
					<a href="<?php echo admin_url( 'admin.php?page=flatsome-panel' ); ?>"
					   class="button button-large"><?php _e( '退出Flatsome面板' ); ?></a>
				</p>
				<?php
			} else {
				?>
				<h1><?php printf( __( '欢迎来到%s安装向导' ), wp_get_theme()); ?></h1>
				<p class="lead"><?php printf( __( '感谢您从ThemeForest选择%s主题。此快速设置向导将帮助您配置新网站。此向导将安装所需的WordPress插件、默认内容、标志，并告诉您一些帮助和支持选项。<br/>应该只需要3-5分钟。' ), wp_get_theme()); ?></p>
				<p><?php _e( '没有时间吗？如果您不想通过向导，您可以跳过并返回到WordPress仪表板。如果你改变主意，随时可以回来！' ); ?></p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"><?php _e( '开始吧' ); ?></a>
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(),'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
						...
						<?php foreach ( $plugins['all'] as $slug => $plugin ) {  ?>
							<li data-slug="<?php echo esc_attr( $slug );?>"><?php echo esc_html( $plugin['name'] );?>
								<span>
    								<?php
								    $keys = array();
								    if ( isset( $plugins['install'][ $slug ] ) ) { $keys[] = '安装'; }
								    if ( isset( $plugins['update'][ $slug ] ) ) { $keys[] = '更新'; }
								    if ( isset( $plugins['activate'][ $slug ] ) ) { $keys[] = '激活'; }
								    echo implode( '和',$keys ).' 必须';
								    ?>
    							</span>
		...
		public function envato_setup_default_content() {
			...
			<form method="post">
				<?php if($this->is_possible_upgrade()){ ?>
					<p><?php _e('您似乎已经在此网站上安装了内容。如果你想安装默认演示内容，你可以在下面选择它。否则只需选择升级选项，以确保一切都是最新的。'); ?></p>
				<?php }else{ ?>
		...
		private function _make_child_theme( $new_theme_title ) {
				...
				if($parent_theme_template !== $new_theme_name){
					echo '<p class="lead success">子主题<strong>'.$new_theme_title.'</strong>已创建并激活！文件夹位于wp-content/themes/<strong>'.$new_theme_name.'</strong></p>';
...
//									echo "Post meta $meta_key was $meta_val \n\n";
		...
		public function envato_setup_logo_design() {
			...
			<form method="post">
				...
				<p><strong>注意：这对最新的新安装最有效。</strong><br/>* 不包括图像。 您需要用您自己的图像替换虚拟图像。
				所有页面都包含在演示内容中，所以您在主题选项中混合和匹配。
				</p>
		...
		public function envato_setup_updates() {
			...
			<p class="lead">输入购买代码以获取自动更新和服务支持</p>
				<?php
				    ...
				    if ( ! empty( $purchase_code ) ) {
				      if ( ! empty( $errors ) ) {
				        ...
				        echo '<div class="notice-warning notice-alt"><p>' . esc_html__( '购买代码未更新。 我们会保留现有的。' ) . '</p></div>';
				      } else {
				        //this means a valid purchase code is present and no errors were found
				       echo '<div class="notice-success notice-alt notice-large" style="margin-bottom:15px!important">' . __( '你的<strong>购买代码有效</strong>。谢谢！享受Flatsome主题和自动更新。' ) . '</div>';
				      }
				    }

				    if ( empty( $purchase_code ) ) {
				    echo '<form class="wupdates_purchase_code" action="" method="post">' .
				             __( '<p>查找如何<a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank">获取购买代码</a> </p>' ) .
				             ...
				             '<input type="text" id="' . sanitize_title( $slug ) . '_wup_purchase_code" name="' . sanitize_title( $slug ) . '_wup_purchase_code"
				              value="' . $purchase_code . '" placeholder="购买代码 ( 例如：9g2b13fa-10aa-2267-883a-9201a94cf9b5 )" style="width:100%; padding:10px;"/><br/><br/>' .
				             '<p class="envato-setup-actions step">' .
...
?>
		...
		?>
			...
			<h1>设置Flatsome子主题（可选）</h1>

			<p>
				如果要更改主题源代码，请使用<a href="https://codex.wordpress.org/Child_Themes" target="_blank">子主题</a>，而不是修改主题HTML / CSS / PHP代码。 这允许父主题接收更新，而不覆盖源代码更改。 使用下面的表单创建和激活子主题。
			</p>

			<?php if(!isset($_REQUEST['theme_name'])){ ?>
			<p class="lead">如果你不知道什么是子主题，请点击"跳过此步骤"按钮</p>
			<?php } ?>
		...
		public function envato_setup_help_support() {
			?>
			<h1>帮助和支持</h1>
			<p class="lead">这个主题来自购买日期的6个月物品支持（可以选择延长这段时间）。 此许可证允许您在单个网站上使用此主题。 请购买额外的许可证以在另一个网站上使用此主题。</p>

			<p class="success">项目支持<strong>文档</strong>包含：</p>
			...
			<p class="error">项目支持<strong>不</strong>包含：</p>
			<ul>
				<li>定制服务（这可以通过<a href="http://studiotracking.envato.com/aff_c?offer_id=4&aff_id=1564&source=DemoInstall" target="_blank">Envato工作室</a>）</li>
				<li>安装服务（这可以通过<a href="http://studiotracking.envato.com/aff_c?offer_id=4&aff_id=1564&source=DemoInstall" target="_blank">Envato工作室</a>）</li>
				<li>帮助和支持非捆绑的第三方插件（即您以后安装的插件）</li>
			</ul>
			<p>有关项目支持的更多详细信息，请参阅ThemeForest<a href="http://themeforest.net/page/item_support_policy" target="_blank">项目支持政策。</a>. </p>
			<p class="envato-setup-actions step">
		...
		public function envato_setup_ready() {
			...
			<p class="lead success">恭喜！ 主题已激活，您的网站已准备就绪。 登录您的WordPress仪表板进行更改和修改任何默认内容，以满足您的需要。</p>
			<p>如果您对这个主题感到满意，<a href="http://themeforest.net/downloads" target="_blank">请回来并留下5星评级。</a><br/>在推特关注<a  href="https://twitter.com/uxthemes" target="_blank">@uxthemes</a>查看更新，谢谢！</p>
