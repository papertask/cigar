	...
	if ( 'blocks' == $typenow && isset( $_GET["post"] ) ) {
		...
		<script>
          jQuery(document).ready(function ($) {
            ...
            $('#submitdiv').
              after('<div class="postbox"><h2 class="hndle">简码</h2><div class="inside"><p><textarea style="width:100%; max-height:30px;">[block id="' + block_id +
                '"]</textarea></p></div></div>')
          })
		</script>
		<?php
...
function block_shortcode( $atts, $content = null ) {
	...
	if ( $post_id ) {
		...
		if ( empty( $html ) ) {
            $html = '<p class="lead shortcode-error">在四亩地生成器中打开以添加和编辑内容</p>';
		}
		...
		if ( isset( $post ) && current_user_can( 'edit_pages' )
			...
			$html              = '<div class="block-edit-link" data-title="编辑区块：' . get_the_title( $post_id ) . '"   data-backend="' . esc_url( $edit_link_backend )
			                     . '" data-link="' . esc_url( $edit_link ) . '"></div>' . $html . '';
		}
	} else {
		$html = '<p><mark>区块<b>"' . esc_html( $id ) . '"</b>未找到</mark></p>';
	}
