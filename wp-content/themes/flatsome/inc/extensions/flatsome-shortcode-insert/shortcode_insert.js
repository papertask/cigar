(function() {
    tinymce.PluginManager.add('ux_shortcode_insert', function( editor, url ) {
        editor.addButton( 'ux_shortcode_insert', {
           text: '简码',
            type: 'menubutton',
            tooltip: '添加简码和内容',
            menu: [
                 {
                    text: '栏目/背景',
                      icon: 'icon dashicons-align-none',
                    onclick: function() {
                        ux_add_section(editor);
                    },
                },
                {
                    text: '行/列',
                     icon: 'icon dashicons-text',
                    onclick: function() {
                        ux_add_row(editor);
                    },
                },
                {
                    text: 'UX幻灯片',
                    icon: 'icon  dashicons-images-alt2',
                    menu: [
                                {
                                    text: '简单幻灯片',
                                    value: '[ux_slider timer="4500" arrows="true" bullets="true" auto_slide="true" nav_color="dark"]<br> 在此处插入幻灯片（四亩地横幅或栏目） <br>[/ux_slider]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: '列幻灯片',
                                    value: '[ux_slider timer="4500" arrows="true" bullets="false" auto_slide="true" nav_color="dark"]<p>[col span="1/4"] 内容 [/col]<br/><br>[col span="1/4"] 内容 [/col]<br/><br>[col span="1/4"] 内容 [/col]<br/><br>[col span="1/4"] 内容 [/col]<br/><br>[col span="1/4"] 内容 [/col]<br/></p>[/ux_slider]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: 'Logo/品牌幻灯片',
                                    value: '[ux_slider height="60px" arrows="true" bullets="false" nav_color="dark" margin="0px"]<br><br/>[logo title="Logo标题" img="http://imageurl" height="60px"]<br/><br/>[logo title="Logo标题" img="http://imageurl" height="60px"]<br/><br/>[logo title="Logo标题" img="http://imageurl" height="60px"]<br/><br/>[logo title="Logo标题" img="http://imageurl" height="60px"]<br/><br/>[logo title="Logo标题" img="http://imageurl" height="60px"]<br/>[logo title="Logo标题" img="http://imageurl" height="60px"]<br/><br/>[logo title="Logo标题" img="http://imageurl" height="60px"]<br/><br/>[/ux_slider]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },

                            ]   
                },
                {
                    text: 'UX横幅',
                    icon: 'icon dashicons-format-image',
                    onclick: function() {
                        ux_add_uxbanner(editor);
                    },
                },
                {
                    text: '元素',
                     icon: 'icon dashicons-screenoptions',
                    onclick: function() {
                        editor.insertContent(this.value());
                    },
                    menu: [

                        {
                            text: '图标框',
                            menu: [
                             {
                                    text: '图标上居中',
                                    value: '[featured_box title="标题文字" img="http://iconurl" img_width="" pos="center" link=""]<p>推荐框文字</p>[/featured_box]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                     text: '图片上',
                                    value: '[featured_box title="标题文字" img="http://iconurl" img_width="" link=""]<p>推荐框文字</p>[/featured_box]',
                                 onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                   text: '图标左',
                                    value: '[featured_box title="标题文字" pos="left" img="http://iconurl" img_width="" link=""]<p>推荐框文字</p>[/featured_box]',
                                   onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                               
                            ]   
                        },
                        {
                            text: '按钮',
                           menu: [
                                {
                                    text: '按钮 - 主要',
                                    value: '[button size="medium" style="primary"  text="中按钮" link="http://" target=""]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: '按钮 - 次要',
                                    value: '[button size="medium" style="secondary"  text="中按钮" link="http://" target=""]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                 {
                                    text: '按钮 - 提醒',
                                    value: '[button size="medium" style="alert"  text="中按钮" link="http://" target=""]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                 {
                                    text: '按钮 - 成功',
                                    value: '[button size="medium" style="success"  text="中按钮" link="http://"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                 {
                                    text: '按钮 - 白',
                                    value: '[button size="medium" style="white"  text="中按钮" link="http://"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                 {
                                    text: 'Alt按钮 - 主要 ',
                                    value: '[button size="medium" style="is-outline primary"  text="中按钮" link="http://" target=""]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: 'Alt按钮 - 次要',
                                    value: '[button size="medium" style="is-outline secondary"  text="中按钮" link="http://" target=""]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: 'Alt按钮 - 成功',
                                    value: '[button size="medium" style="is-outline success"  text="中按钮" link="http://"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: 'Alt按钮 - 白',
                                    value: '[button size="medium" style="is-outline white"  text="中按钮" link="http://"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                               
                            ]    
                        },
                        {
                            text: '选项卡/手风琴',
                            onclick: function(e) {
                            },
                            menu: [
                                {
                                    text: '选项卡 - 水平',
                                    value: '[tabgroup title="选项卡组标题"]<br>[tab title="选项卡1标题"] 选项卡内容 [/tab]<br>[tab title="选项卡2标题"] 选项卡内容 [/tab]<br>[tab title="选项卡3标题"] 选项卡内容 [/tab]<br>[/tabgroup]<br>',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                 {
                                    text: '选项卡 - 水平居中',
                                    value: '[tabgroup style="center" title=""]<br>[tab title="选项卡1标题"] 选项卡内容 [/tab]<br>[tab title="选项卡2标题"] 选项卡内容 [/tab]<br>[tab title="选项卡3标题"] 选项卡内容 [/tab]<br>[/tabgroup]<br>',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                 {
                                    text: '选项卡 - 水平丸状',
                                    value: '[tabgroup style="pills" title=""]<br>[tab title="选项卡1标题"] 选项卡内容 [/tab]<br>[tab title="选项卡2标题"] 选项卡内容 [/tab]<br>[tab title="选项卡3标题"] 选项卡内容 [/tab]<br>[/tabgroup]<br>',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: '选项卡 - 垂直',
                                    value: '[tabgroup_vertical title=""]<br>[tab title="选项卡1标题"] 选项卡内容 [/tab]<br>[tab title="选项卡2标题"] 选项卡内容 [/tab]<br>[tab title="选项卡3标题"] 选项卡内容 [/tab]<br>[/tabgroup_vertical]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: '手风琴',
                                    value: '[accordion title="手风琴标题"]<br><br>[accordion-item title="手风琴项目1标题"]<br>手风琴项目1内容在这里<br>[/accordion-item]<br><br>[accordion-item title="手风琴项目1标题"]<br>手风琴项目1内容在这里<br>[/accordion-item]<br><br>[accordion-item title="手风琴项目1标题"]<br>手风琴项目1内容在这里<br> [/accordion-item]<br><br>[/accordion]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                }
                               
                            ]
                        },
                        {
                            text: '地图',
                            value: '[map lat="40.79028" long="-73.95972" height="500px" color="#58728a"] 在此输入地图内容.. [/map]',
                            onclick: function(e) {
                                e.stopPropagation();
                                editor.insertContent(this.value());
                            }       
                        },
                        {
                            text: '博客文章',
                            menu: [
                                {
                                    text: '博客文章幻灯片',
                                    value: '[blog_posts posts="6" columns="3" image_height="200px" show_date="true"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                 {
                                    text: '博客文章幻灯片 - 文字叠加',
                                    value: '[blog_posts columns="3" style="text-overlay" posts="8" category="" image_height="150px" show_date="true"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                 {
                                    text: '博客文章幻灯片 - 文本反弹',
                                    value: '[blog_posts columns="3" style="text-bounce" posts="8" category="" image_height="150px" show_date="true"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                }
                            ]  
                               
                        },
                        {
                            text: '标题/分隔',
                            value: '来自子子菜单的文本',
                            menu: [
                                {
                                    text: '标题 - 左',
                                    value: '[title text="这是一个标题" link="" link_text=""]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: '标题 - 居中',
                                    value: '[title text="这是一个居中标题" style="center"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: '标题 - 粗左',
                                    value: '[title text="这是粗标题" style="bold"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: '标题 - 粗居中',
                                    value: '[title text="这是粗体居中标题" style="bold_center"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: '标题 - 左 (带链接)',
                                    value: '[title text="这是标题" link="#" link_text="标题链接"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: '分隔',
                                    value: '[divider]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: '分隔 - 全宽',
                                    value: '[divider width="full"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: '分隔 - 小',
                                    value: '[divider width="small"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                     text: '分隔 - 中',
                                    value: '[divider width="medium"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                     text: '间隙 - 30px',
                                    value: '[gap height="30px"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                     text: '间隙 - 60px',
                                    value: '[gap height="60px"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                     text: '间隙 - 15px',
                                    value: '[gap height="15px"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                }

                            ]   
                        },
                        {
                            text: '关注/分享',
                            menu: [
                             {
                                    text: '关注图标',
                                    value: '[follow twitter="http://" facebook="http://" email="email@post.com" pinterest="http://" rss="http://" instagram="http://" googleplus="http://"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: '关注图标 - 小',
                                    value: '[follow size="small" twitter="http://" facebook="http://" email="email@post.com" pinterest="http://" rss="http://" instagram="http://" googleplus="http://"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: '分享图标',
                                    value: '[share title=""]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                               
                            ]   
                        },

                        {
                            text: '价格表',
                            value: '[ux_price_table price="99$" title="一个标题" featured="false" description="这是一个描述" button_text="Order Now" button_link="#"]<br>[bullet_item text="这是项目符号项目"]<br>[bullet_item text="这是项目符号项目"]<br>[bullet_item text="这是项目符号项目"]<br>[/ux_price_table]',
                            onclick: function(e) {
                                e.stopPropagation();
                                editor.insertContent(this.value());
                            }       
                        },
                        {
                            text: '客户评价',
                            value: '[testimonial style="" image="http://imageurl" name="作者名字" company="公司名" stars="5"]<br>在此添加客户评价文字<br>[/testimonial]',
                            onclick: function(e) {
                                e.stopPropagation();
                                editor.insertContent(this.value());
                            }       
                        },
                        {
                            text: '团队成员',
                            value: '[team_member name="名字" title="标题" facebook="http://" twitter="http://" pinterest="http://"  img="http://imageurl"]<br>团队成员描述<br>[/team_member]',
                            onclick: function(e) {
                                e.stopPropagation();
                                editor.insertContent(this.value());
                            }       
                        },
                        {
                            text: '信息框',
                            value: '[message_box bg="#hex 或 http://imageurl"] 信息框文字 [/message_box]',
                            onclick: function(e) {
                                e.stopPropagation();
                                editor.insertContent(this.value());
                            }       
                        },
                        {
                            text: '滚动到链接',
                            value: '[scroll_to link="#scrolltothis" title="滚动到此"]',
                            onclick: function(e) {
                                e.stopPropagation();
                                editor.insertContent(this.value());
                            }       
                        },                        
                    ]
                },{
                    text: '商店',
                    icon: 'icon dashicons-cart',
                    onclick: function() {
                        editor.insertContent(this.value());
                    },
                    menu: [
                        {
                            text: '产品幻灯片',
                            menu: [
                            {
                                    text: '推荐产品',
                                    value: '[ux_featured_products products="" columns="4" title="查看我们的推荐产品！"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                             {
                                    text: '畅销产品',
                                    value: '[ux_bestseller_products products="" columns="4" title="查看我们的畅销产品！"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: '自定义分类',
                                    tooltip: '从所选分类显示产品',
                                    value: '[ux_custom_products cat="" products="" columns="4" title="查看我们的产品！"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: '促销产品',
                                    value: '[ux_sale_products columns="4" title="查看我们的促销产品！"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: '最新产品',
                                    value: '[ux_latest_products columns="4" title="查看我们的最新产品！"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                            ]       
                        },
                        {
                            text: '分类',
                            menu: [
                             {
                                    text: '分类幻灯片',
                                    value: '[ux_product_categories number="10" parent="0" columns="4" title="我们的分类" ]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: '分类网格 - 简单网格',
                                    value: '[product_categories number="10" parent="0" columns="4" title="我们的分类"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: '分类网格 - 新网格风格',
                                    value: '[ux_product_categories_grid number="22" parent="0"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                }
                            ]         
                        },
                        {
                            text: '产品看书幻灯片',
                            value: '[product_lookbook cat="category-slug" products="8"]',
                            onclick: function(e) {
                                e.stopPropagation();
                                editor.insertContent(this.value());
                            }       
                        },
                        {
                            text: '产品瀑布流风格网格',
                            value: '[products_pinterest_style products="" cat="category-slug" columns="4"]',
                            onclick: function(e) {
                                e.stopPropagation();
                                editor.insertContent(this.value());
                            }       
                        },
                        {
                            text: 'WooCommerce',
                            menu: [
                             {
                                    text: '订单跟踪',
                                    value: '[woocommerce_order_tracking]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: '产品价格/购物车按钮',
                                    value: '[add_to_cart id="" sku=""]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: 'SKU产品',
                                    value: '[product id="" sku=""]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: '自定义产品',
                                    value: '[product_category category="" per_page="12" columns="4" orderby="date" order="desc"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: '最新产品',
                                    value: '[recent_products per_page="12" columns="4" orderby="date" order="desc"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: '推荐产品',
                                    value: '[featured_products per_page="12" columns="4" orderby="date" order="desc"]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },
                                {
                                    text: '商店信息',
                                    value: '[woocommerce_shop_messages]',
                                    onclick: function(e) {
                                        e.stopPropagation();
                                        editor.insertContent(this.value());
                                    }       
                                },




                            ]         
                        },

                    ]
                },
           ]
        });
    });



/* ADD BANNER */
function ux_add_uxbanner(editor){
    var list_style = 'height:90px; width:130px; text-indent: -999px; text-align:center;padding-top:3px; background-repeat:no-repeat;font-size:10px!important;border-top:1px solid #555;';
    ux_selected_image = 'http://imageurl';

    editor.windowManager.open( {
        title: '插入横幅',
        body: [
        {
            type: 'listbox', 
            name: 'height', 
            label: '高度',
            tooltip: '设置横幅高度',
            'values': [
                {text: '500px', value: 'height="500px"'},
                {text: '700px', value: 'height="700px"'},
                {text: '600px', value: 'height="600px"'},
                {text: '400px', value: 'height="400px"'},
                {text: '300px', value: 'height="300px"'},
                {text: '200px', value: 'height="200px"'},
                {text: '100px', value: 'height="100px"'},

            ]
        },
        {
            type: 'button', 
            name: 'button', 
            label: '背景图片',
            text: '选择图片',
            icon: 'icon dashicons-format-gallery',
             onclick: function(e) {
               e.preventDefault();
               UXgalleryModal();
            },
        },
        {
            type: 'listbox', 
            name: 'parallax', 
            label: '背景视差', 
            'values': [
                {text: '禁用', value:'parallax="0"'},
                {text: '低', value: 'parallax="1"'},
                {text: '正常', value: 'parallax="3"'},
                {text: '更强', value: 'parallax="6"'},
                {text: '最强', value: 'parallax="9"'},
            ]
        },

        {
            type: 'listbox', 
            name: 'parallax_text', 
            label: '文本视差', 
            'values': [
                {text: '禁用', value: 'parallax="0"'},
                {text: '低', value: 'parallax="1"'},
                {text: '正常', value: 'parallax_="3"'},
                {text: '更强', value: 'parallax="6"'},
                {text: '最强', value: 'parallax="9"'},
            ]
        },
        {
            type: 'listbox', 
            name: 'text_animation', 
            label: '文字动画', 
            'values': uxAnimations
        },
        {
            type: 'listbox', 
            name: 'text_color', 
            label: '文字颜色', 
            tooltip: '选择文字浅色/深色',
            'values': [
                {text: '浅色文字', value: 'text_color="light"', style: list_style+' background-image: url("data:image/png;base64, ");'},
                {text: '深色文字', value: 'text_color="dark"', style: list_style+' background-image: url("data:image/png;base64, ");'},
                
            ]
        },
        {
            type: 'listbox', 
            name: 'text_layout', 
            label: '文字布局', 
            tooltip: '选择预定义的文本位置',
            'values': [
                {text:'文字居中', value: 'text_align="center" text_pos="center" text_width="70%"', style: list_style+' background-image: url("data:image/png;base64, ");'},
                {text:'文字居左', value: 'text_align="left" text_pos="left center" text_width="40%" text_bg=""', style: list_style+' background-image: url("data:image/png;base64, ");'},
                {text:'文字居左中', value: 'text_align="center" text_pos="left center" text_width="40%" text_bg=""', style: list_style+' background-image: url("data:image/png;base64, ");'},
                {text:'文字居右', value: 'text_align="right" text_pos="right center" text_width="40%" text_bg=""', style: list_style+' background-image: url("data:image/png;base64, ");'},
                {text:'文字右中', value: 'text_align="center" text_pos="right center" text_width="40%" text_bg=""', style: list_style+' background-image: url("data:image/png;base64, ");'},
                {text:'文字下中', value: 'text_align="center" text_pos="bottom center" text_width="40%" text_bg=""', style: list_style+' background-image: url("data:image/png;base64, ");'},
                {text:'文字上左', value: 'text_align="left" text_pos="left top" text_width="40%" text_bg=""', style: list_style+' background-image: url("data:image/png;base64, ");'},
                {text:'文字上居中', value: 'text_align="center" text_pos="top center" text_width="70%" text_bg=""', style: list_style+' background-image: url("data:image/png;base64, ");'},
                {text:'框式文字左', value: 'text_align="left" text_pos="left center" text_width="40%" text_bg="#000" text_bg_opacity=0.8" padding="30px"', style: list_style+' background-image: url("data:image/png;base64, ");'},
                {text:'框式文字右', value: 'text_align="left" text_pos="right center" text_width="40%" text_bg="#000" text_bg_opacity=0.8" padding="30px"', style: list_style+' background-image: url("data:image/png;base64, ");'},
                {text:'框式文字居中', value: 'text_align="center" text_pos="center" text_width="40%" text_bg="#000" text_bg_opacity=0.8" padding="30px"', style: list_style+' background-image: url("data:image/png;base64, ");'},
                {text:'框式徽章风格', value: 'text_align="center" text_pos="bottom full-width" text_bg="#000" text_bg_opacity=0.8" padding="10px"', style: list_style+' background-image: url("data:image/png;base64, ");'},
            ]
        },
        {
            type: 'listbox', 
            name: 'text_content', 
            label: '文字内容', 
            tooltip :'选择预定义的文本，您可以在编辑器中更改这个',
            'values': [
                {text: '默认', value: '<h4 class="alt-font">较小在上面</h4><p>___</p><h1>大标题</h1><h4 class="thin-font">标题下的较小文本</h4><p>[button text="button" link="#" size="medium" style="is-outline white"]</p>', style: list_style+' background-image: url("data:image/png;base64, ");'},
                {text: '大横幅', value: '<h3 class="alt-font">较小在上面</h3><p>____</p><h1 class="h-large">大标题</h1><h4 class="thin-font">标题下的较小文本</h4><p>[button text="button" link="#" size="medium" style="is-outline white"] <br>[button text="button" link="#" size="medium" style="is-outline white"] <br>[button text="button" link="#" size="medium" style="is-outline white"]</p>', style: list_style+' background-image: url("data:image/png;base64, ");'},
                {text: '简单', value: '<h1>大标题</h1><h4 class="thin-font">标题下的较小文本</h4>', style: list_style+' background-image: url("data:image/png;base64, ");'},
                {text: '简单的两个按钮', value: '<h2>大标题</h2><h4 class="thin-font">标题下的较小文本</h4><p>[button text="button" link="#" size="medium" style=""] <br>[button text="button" link="#" size="medium" style="is-outline"]</p>', style: list_style+' background-image: url("data:image/png;base64, ");'},
                {text: '边框标题', value: '<h2 class="text-bordered-white">大标题</h2><h4 class="thin-font">标题下的较小文本</h4>', style: list_style+' background-image: url("data:image/png;base64, ");'},
            ]
        },
        {
            type: 'listbox', 
            name: 'banner_effect', 
            label: '效果', 
            'values': [
                {text: '无效果', value: 'effect=""'},
                {text: '雪花', value: 'effect="snow"'},
                {text: '五彩纸屑', value: 'effect="confetti"'},
                {text: '滑动玻璃', value: 'effect="sliding-glass"'},
                {text: '火花', value: 'effect="sparkle"'},
                {text: '下雨', value: 'effect="rain"'},
            ]
        },
    ],
        onsubmit: function( e ) {
            var banner_content = '[ux_banner '+e.data.parallax+' bg="'+ux_selected_image+'" '+e.data.height+' '+e.data.banner_effect+'][text_box '+e.data.text_color+' '+e.data.text_layout+'  '+e.data.parallax_text+' '+e.data.text_animation+'] '+e.data.text_content+' [/text_box] [/ux_banner]';
            if(e.data.text_color == 'text_color="dark"'){
              var banner_content = banner_content.replace(/white/g, "dark");
              var banner_content = banner_content.replace(/#000/g, "#fff");
            }
            editor.insertContent(banner_content);
        }
    });
}
// END BANNERS


/* ADD SECTION */
function ux_add_section(editor){
    ux_selected_image = 'http://imageurl';

    editor.windowManager.open( {
        title: '插入栏目',
        body: [
        {
            type: 'textbox', 
            name: 'padding', 
            label: '填充',
            value: '30px',
        },
        {
            type: 'button', 
            name: 'button', 
            label: '背景图片',
            text: '选择图片',
            icon: 'icon dashicons-format-gallery',
             onclick: function(e) {
               e.preventDefault();
               UXgalleryModal();
            },
        },
        {
            type: 'listbox', 
            name: 'parallax', 
            label: '背景视差', 
            'values': [
                {text: '禁用', value:'parallax="0"'},
                {text: '低', value: 'parallax="1"'},
                {text: '正常', value: 'parallax="3"'},
                {text: '更强', value: 'parallax="6"'},
                {text: '最强', value: 'parallax="9"'},
            ]
        },
        {
            type: 'listbox', 
            name: 'parallax_text', 
            label: '文字视差', 
            'values': [
                {text: '禁用', value: 'parallax_text="0"'},
                {text: '低', value: 'parallax_text="1"'},
                {text: '正常', value: 'parallax_text="3"'},
                {text: '更强', value: 'parallax_text="6"'},
                {text: '最强', value: 'parallax_text="9"'},
            ]
        },
        {
            type: 'textbox', 
            name: 'margin', 
            label: '下边距',
            value: '0px',
        },
        {
            type: 'listbox', 
            name: 'text_color', 
            label: '文字颜色', 
            'values': [
                {text: '正常', value: ''},
                {text: '浅色（用于深色背景）', value: 'dark="true"'},
            ]
        }

    ],
        onsubmit: function( e ) {
            editor.insertContent( '[section bg="'+ux_selected_image+'" title="" padding="'+e.data.padding+'" '+e.data.text_color+' '+e.data.parallax_text+' '+e.data.parallax+' margin="'+e.data.margin+'"]<br><br>[/section]');
        }
    });
}
// END SECTIIONS


/* ADD ROW */
function ux_add_row(editor){
    editor.windowManager.open( {
        title: '插入行',
        body: [
        {
            type: 'listbox', 
            name: 'columns', 
            label: '列', 
            'values': [
                {text: '1/1 (全宽)', value: '1'},
                {text: '1/2 | 1/2', value: '2'},
                {text: '1/3 | 1/3 | 1/3', value:'3'},
                {text: '1/4 | 1/4 | 1/4 | 1/4', value:'4'},
                {text: '1/6 | 1/6 | 1/6 | 1/6 | 1/6 | 1/6', value: '6'},
                {text: '1/3 | 2/3', value:'1-3'},
                {text: '2/4 | 1/4 | 1/4', value:'2-1-1'},
            ]
        },
        {
            type: 'listbox', 
            name: 'column_animate', 
            label: '动画列？', 
            'values': uxAnimations
              
            
        },
        {
            type: 'listbox', 
            name: 'columns_hover', 
            label: '列悬停效果', 
            'values': [
                {text: '无效果', value:' '},
                {text: '焦点', value: 'hover="focus"'},
                {text: '淡入', value:'hover="fade"'},
                {text: '模糊', value: 'hover="blur"'},
                {text: '灰度', value: 'hover="grayscale"'},
            ]
        },
        {
            type: 'listbox', 
            name: 'row_style', 
            label: '列风格', 
            'values': [
                {text: '默认', value:' '},
                {text: '折叠（无填充）', value: 'style="collapse"'},
                {text: '用线划分', value:'style="divided"'},
                {text: '框式风格', value:'style="boxed"'},
            ]
        },
         {
            type: 'listbox', 
            name: 'column_content', 
            label: '列内容', 
            'values': [
                {text: '空白', value:'在此插入内容...<br>'},
                {text: '文字', value:'<h3>在此添加标题</h3>[divider]<p>多位全国人大常委会委员指出，目前环境保护税涵盖的税目范围较窄，建议适当拓宽其征收范围</p>'},
                {text: '推荐框 - 图标在上', value:'[featured_box title="标题文字" img="http://iconurl" img_width="" pos="center" link=""]<p>推荐框文字</p>[/featured_box]<br>'},
                {text: '推荐框 - 图片在左', value:'[featured_box title="标题文字" img="http://iconurl" img_width="" pos="left" link=""]<p>推荐框文字</p>[/featured_box]<br>'},
                {text: '团队成员', value:'[team_member name="名字" title="标题" facebook="http://" twitter="http://" pinterest="http://"  img="http://imageurl"]<br>团队成员描述<br>[/team_member]<br>'},
                {text: '价格表', value:'[ux_price_table title="一个标题" featured="false" description="这是一个标题" button_text="立即购买" button_link="#"]<br>[bullet_item text="这是符号项目"]<br>[bullet_item text="这是符号项目"]<br>[bullet_item text="这是符号项目"]<br>[/ux_price_table]<br>'},
                {text: '客户评价', value:'[testimonial image="http://imageurl" name="作者名" company="公司名" stars="5"]<br>在此添加客户评价文字<br>[/testimonial]<br>'},
            ]
        }

    ],
        onsubmit: function( e ) {
            if(e.data.columns == '1'){
               var insertContent = '[col span="1/1" '+e.data.columns_hover+' '+e.data.column_animate+']<br>'+e.data.column_content+'<br/>[/col]<br>';
            }
             if(e.data.columns == '2'){
               var insertContent = '[col span="1/2" '+e.data.columns_hover+' '+e.data.column_animate+']<br>'+e.data.column_content+'<br/>[/col]<br>[col span="1/2" '+e.data.columns_hover+' '+e.data.column_animate+']<br>'+e.data.column_content+'<br/>[/col]<br>';
            }
            if(e.data.columns == '3'){
               var insertContent = '[col span="1/3" '+e.data.columns_hover+' '+e.data.column_animate+']<br>'+e.data.column_content+'<br/>[/col]<br>[col span="1/3" '+e.data.columns_hover+' '+e.data.column_animate+']<br>'+e.data.column_content+'<br/>[/col]<br>[col span="1/3" '+e.data.columns_hover+' '+e.data.column_animate+']<br>'+e.data.column_content+'<br/>[/col]<br>';
            }
            if(e.data.columns == '4'){
               var insertContent = '[col span="1/4" '+e.data.columns_hover+' '+e.data.column_animate+']<br>'+e.data.column_content+'<br/>[/col]<br>[col span="1/4" '+e.data.columns_hover+' '+e.data.column_animate+']<br>'+e.data.column_content+'<br/>[/col]<br>[col span="1/4" '+e.data.columns_hover+' '+e.data.column_animate+']<br>'+e.data.column_content+'<br/>[/col]<br>[col span="1/4" '+e.data.columns_hover+' '+e.data.column_animate+']<br>'+e.data.column_content+'<br/>[/col]<br>';
            }
            if(e.data.columns == '6'){
               var insertContent = '[col span="1/6" '+e.data.columns_hover+' '+e.data.column_animate+']<br>'+e.data.column_content+'<br/>[/col]<br>[col span="1/6" '+e.data.columns_hover+' '+e.data.column_animate+']<br>'+e.data.column_content+'<br/>[/col]<br>[col span="1/6" '+e.data.columns_hover+' '+e.data.column_animate+']<br>'+e.data.column_content+'<br/>[/col]<br>[col span="1/6" '+e.data.columns_hover+' '+e.data.column_animate+']<br>'+e.data.column_content+'<br/>[/col]<br>[col span="1/6" '+e.data.columns_hover+' '+e.data.column_animate+']<br>'+e.data.column_content+'<br/>[/col]<br>[col span="1/6" '+e.data.columns_hover+' '+e.data.column_animate+']<br>'+e.data.column_content+'<br/>[/col]<br>[col span="1/6" '+e.data.columns_hover+' '+e.data.column_animate+']<br>'+e.data.column_content+'<br/>[/col]<br> ';
            }
            if(e.data.columns == '1-3'){
               var insertContent = '[col span="1/3" '+e.data.columns_hover+' '+e.data.column_animate+']<br>'+e.data.column_content+'<br/>[/col]<br>[col span="2/3" '+e.data.columns_hover+' '+e.data.column_animate+']<br>'+e.data.column_content+'<br/>[/col]<br>';
            }
            if(e.data.columns == '2-1-1'){
               var insertContent = '[col span="2/4" '+e.data.columns_hover+' '+e.data.column_animate+']<br>'+e.data.column_content+'<br/>[/col]<br>[col span="1/4" '+e.data.columns_hover+' '+e.data.column_animate+']<br>'+e.data.column_content+'<br/>[/col]<br>[col span="1/4" '+e.data.columns_hover+' '+e.data.column_animate+']<br>'+e.data.column_content+'<br/>[/col]<br>';
            }
            editor.insertContent('[row '+e.data.row_style+']<p>'+insertContent+'</p>[/row]<br>');
        }
    });
}
// END ROW


var uxAnimations = [
    {text: '无动画', value:''},
    {text: '淡入左', value:'animate="fadeInLeft"'},
    {text: '淡入右', value: 'animate="fadeInRight"'},
    {text: '淡入上', value: 'animate="fadeInUp"'},
    {text: '淡入下', value: 'animate="fadeInDown"'},
    {text: '反弹', value: 'animate="bounceIn"'},
    {text: '反弹上', value: 'animate="bounceInUp"'},
    {text: '反弹下', value: 'animate="bounceInDown"'},
    {text: '反弹左', value: 'animate="bounceInLeft"'},
    {text: '反弹右', value: 'animate="bounceInRight"'},
    {text: '旋转左', value: 'animate="rotateInUpLeft"'},
    {text: '旋转右', value: 'animate="rotateInDownRight"'},
    {text: '水平翻转', value: 'animate="flipInX"'},
    {text: '垂直翻转', value: 'animate="flipInY"'},
]


function UXgalleryModal(){
    // Uploading files
var file_frame;
 
    // If the media frame already exists, reopen it.
    if ( file_frame ) {
      file_frame.open();
      return;
    }
 
    // Create the media frame.
    file_frame = wp.media.frames.file_frame = wp.media({
      title: jQuery( this ).data( 'uploader_title' ),
      button: {
        text: jQuery( this ).data( 'uploader_button_text' ),
      },
      multiple: false  // Set to true to allow multiple files to be selected
    });
 
    // When an image is selected, run a callback.
    file_frame.on( 'select', function() {
      // We set multiple to false so only get one image from the uploader
      attachment = file_frame.state().get('selection').first().toJSON();
      ux_selected_image = attachment.url;
      jQuery('.mce-i-icon.dashicons-format-gallery').parent().addClass('image-bg-button');
      jQuery('.mce-i-icon.dashicons-format-gallery').parent().css('background-image','url('+attachment.url+')');
      // Do something with attachment.id and/or attachment.url here
    });
 
    // Finally, open the modal
    file_frame.open();
}

})();
