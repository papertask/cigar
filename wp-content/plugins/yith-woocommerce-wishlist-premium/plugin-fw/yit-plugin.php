...
if( ! function_exists( 'yit_plugin_fw_row_meta' ) ){
	...
	function yit_plugin_fw_row_meta( $plugin_meta, $plugin_file, $plugin_data, $status ){
		...
		$default = array(
			'live_demo' => array(
				'label' => _x( '在线演示', 'Plugin Row Meta', 'yith-plugin-fw' ),
				'icon'  => 'dashicons  dashicons-laptop',
			),

			'documentation' => array(
				'label' => _x( '文档', 'Plugin Row Meta', 'yith-plugin-fw' ),
				'icon'  => 'dashicons  dashicons-search',
			),

			'support' => array(
				'label' => _x( '支持', 'Plugin Row Meta', 'yith-plugin-fw' ),
				'icon'  => 'dashicons  dashicons-admin-users',
			),

			'premium_version' => array(
				'label' => _x( '高级版本', 'Plugin Row Meta', 'yith-plugin-fw' ),
...
if( ! function_exists( 'yith_add_action_links' ) ){
	...
	function yith_add_action_links( $links, $panel_page = '', $is_premium = false ) {
		if( ! empty( $panel_page )  ){
			$links[] = sprintf( '<a href="%s">%s</a>', admin_url( "admin.php?page={$panel_page}" ), _x( '设置', 'Action links',  'yith-plugin-fw' ) );
		}
